...
This Environment Template is a boilerplate for **creating a production environment** based on a stack using Java 17 with [Quarkus](https://quarkus.io/) for exposing the REST API, basic [ReactJS](https://react.dev/) for frontend and [Postgres](https://www.postgresql.org/) as the database.
...
- `react-app` for frontend, based on a `node` image (1 Core CPU and 512 MB RAM)
- `quarkus-applicaton` for backend, also based on a `ubi9/openjdk17` image (1 Core CPU and 512 MB RAM)
- `postgres-db` using a `postgres` image (2 Core CPU and 1 GB RAM)
  
and 1 persistent volume:
  
- `postgres-volume` (1 GB disk space)
